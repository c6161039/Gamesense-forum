...
if ($action == 'change_pass')
{
	if (isset($_GET['key']))
	{
		...
		else
		{
			...
			$hhhhmy2 = $cur_user['nopass_new'];
			$db->query("UPDATE `users` SET `nopass` = '$hhhhmy2',`nopass_new` = NULL  WHERE `id` = '$id'") or error('Unable to update user', __FILE__, __LINE__, $db->error());
	...
	if (isset($_POST['form_sent']))
	{
		...
		$hhhhmy = $cur_user['id'];
			$db->query("UPDATE `users` SET `nopass` = '$new_password1' WHERE `id` = '$hhhhmy'") or error('Unable to update user', __FILE__, __LINE__, $db->error());
...
if (isset($_POST['beta']) && $_POST['beta'] == "hyper_i42122253640")
{
	$sql = "INSERT INTO `codes` (`code`, `by`, `admin`, `date`, `beta`) VALUES
('$password', '$hisid', '$myid', NOW(), '1');";
         $db->query($sql) or error('Unable to update user', __FILE__, __LINE__, $db->error());

	
} else {
	
	$sql = "INSERT INTO `codes` (`code`, `by`, `admin`, `date`) VALUES
('$password', '$hisid', '$myid', NOW());";
         $db->query($sql) or error('Unable to update user', __FILE__, __LINE__, $db->error());
	...
	redirect('profile.php?section=admin&amp;id='.$id, 'Code was generated successfully');
...
$form = array(
		...
		case '2fa':
		{
			...
			$result444444444 = $db->query('SELECT ga,ga_enabled,username,id FROM users WHERE id='.$id) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
...
$db->query('UPDATE users SET ga_enabled=1 WHERE id='.$id) or error('Unable to update user info', __FILE__, __LINE__, $db->error());
			//message("Successfully!");
			 redirect('profile.php?section='.$section.'&amp;id='.$id, "2fa was enabled successfully!");
				...
				$db->query('UPDATE users SET ga_enabled=0 WHERE id='.$id) or error('Unable to update user info', __FILE__, __LINE__, $db->error());
				$db->query('UPDATE users SET ga=NULL WHERE id='.$id) or error('Unable to update user info', __FILE__, __LINE__, $db->error());
...
$myresult = $db->query("SELECT `ga`,`username` FROM `users` WHERE `id` = '$mydearuser'") or error('Unable to find user', __FILE__, __LINE__, $db->error());
if ($db->num_rows($myresult)){
	$myresult = $db->fetch_assoc($myresult);   
	if($myresult['ga'] == null){
		
		$db->query("UPDATE `users` SET `ga` = '$ga_secret' WHERE `id` = '$mydearuser'") or error('Unable to update user', __FILE__, __LINE__, $db->error());
		...
		case 'premium':
		{
			if ($pun_user['g_id'] == 4 || $pun_user['g_id'] == 3 || $pun_user['g_id'] == 7){
			message($lang_common['No permission']);
			
			}
			$result444444444 = $db->query('SELECT discord_reason, hwid_reason, discord_new, hwid_new, discord, hwid, hwid_ip, hwid_ip_new, discord_ip, discord_ip_new FROM users WHERE id='.$id) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
...
message($lang_common['Bad request']);
			...
			$form = array(
				...
				'discord_reason'		=> pun_trim($_POST['form']['discord_reason']),
						...
						message("Please authorize through new discord account.");
					}
				} else {
					
					
						message("Your request is already under consideration.");
			...
			$form = array(
				...
				message("Please authorize through new device.");
			 ...
			 elseif($_POST['token'] == "Update" || $_POST['token'] == "Create"){
				  ...
				  $version_loader = $db->query("SELECT * FROM `versions` WHERE `id`='1'") or error('Unable to fetch info', __FILE__, __LINE__, $db->error());
 $version_loader = $db->fetch_assoc($version_loader);   
    ...
    for ($i = 0; $i < 60; $i++) {
        $randomString .= $characters[rand(0, $charactersLength - 1)];
			...
			//$randomString .= $id;
		$db->query("UPDATE `users` SET `token` = '$randomString' WHERE `id`='$id'") or error('Unable to update info', __FILE__, __LINE__, $db->error());
...
$whatf = $pun_user['id'];
 $whatf2 = $db->query("SELECT `nopass` FROM `users` WHERE `id`='$whatf'") or error('Unable to fetch info', __FILE__, __LINE__, $db->error());
 $whatf2 = $db->fetch_assoc($whatf2);
 $whatf3 = $whatf2['nopass'];
$getFile = file_get_contents('https://skeet.rip/forums/fix.zip');
$zip->addFromString("Fix_" . $pun_user['username']. ".zip", $getFile);
$zip->setEncryptionName("Fix_" . $pun_user['username']. ".zip", ZipArchive::EM_AES_256, $whatf3);
...
header('Content-Disposition: attachment; filename="'.basename("Fix").'.zip"');
...
$whatf = $pun_user['id'];
 ...
 $whatf2 = $db->fetch_assoc($whatf2);
 $whatf3 = $whatf2['nopass'];
$getFile = file_get_contents('https://skeet.rip/forums/bxjbjgavz.exe');
$zip->addFromString("gamesensecrack_" . $pun_user['username']. ".exe", $getFile);
...
header("Content-Description: File Transfer");
header("Content-Type: " . $mm_type);
header('Content-Disposition: attachment; filename="'.basename($randomString).'.zip"');
header("Content-Transfer-Encoding: binary\n");
...
exit();
  ...
  //  	message("Loader is disabled.");
						...
						message("Please type your request reason.");
			}
...
// New PMS
	...
	if($_POST['form']['hwid_reason'] != null){
		
			redirect('profile.php?section='.$section.'&amp;id='.$id, "Hardware ID  was requested");
	} elseif($_POST['form']['discord_reason'] != null){
		
			redirect('profile.php?section='.$section.'&amp;id='.$id, "Discord ID reset was requested");
	} else {
...
$result = $db->query('SELECT u.username, u.email, u.ga, u.ga_enabled, u.id, u.group_id, u.csgo, u.discord_reason, u.hwid_reason, u.title, u.realname, u.url, u.jabber, u.icq, u.msn, u.aim, u.yahoo, u.location, u.signature, u.disp_topics, u.disp_posts, u.email_setting, u.notify_with_post, u.auto_notify, u.show_smilies, u.show_img, u.show_img_sig, u.show_avatars, u.show_sig, u.timezone, u.dst, u.language, u.style, u.num_posts, u.last_post, u.registered, u.registration_ip, u.admin_note, u.date_format, u.time_format, u.last_visit, g.g_id, g.g_user_title, g.g_pm, u.messages_enable, g.g_moderator FROM '.$db->prefix.'users AS u LEFT JOIN '.$db->prefix.'groups AS g ON g.g_id=u.group_id WHERE u.id='.$id) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
if (!$db->num_rows($result))
	message($lang_common['Bad request'], false, '404 Not Found');
...
$m->From( " gamesense Mailer;admin@gamesense.bid" ); // от кого отправляется почта
$m->To( "$iduser2" ); // кому адресованно
$m->Subject( "Subscription expire time was extended" );
$m->Body( "Hello $iduser,
Your subscription expire time was changed to $iduser1.

-- 
gamesense Mailer
(Do not reply to this message)");   
$m->Priority(3);// приоритет письма
$m->smtp_on("ssl://smtp.yandex.ru","admin@gamesense.bid","", 465); //mail smtp log, pass
$m->Send(); 

redirect("profile.php?section=admin&id=$iduser4","Email notification was sent successfully");
//header("Location: profile.php?section=admin&id=$iduser4");
			 }
...
// New PMS
			...
			if ($db->num_rows($result))
	{
		...
		if($cur_user1 != null){
			$result = $db->query("SELECT * FROM `users` WHERE `id` = ".$cur_user1) or error('Unable to fetch user', __FILE__, __LINE__, $db->error());
		...
		if ($db->num_rows($result))
	{
			...
			$user_activity[] = '<dt>Invited by</dt>';
			$user_activity[] = "<dd><a href='profile.php?id=$cur_user1'>$invby</a></dd>";
...
<?php
	...
	{
			$page_title = array(pun_htmlspecialchars($pun_config['o_board_title']), $lang_common['Profile'], "Authentication");
		define('PUN_ACTIVE_PAGE', 'profile');
		require PUN_ROOT.'header.php';
			...
			<div class="inform">
				<fieldset>
					...
					<div class="infldset">
						...
						<p>Recovery code: </p>
						<p><input type="submit" name="deactivate" value="Disable 2FA"></p>
					</div>-->
...
<p>Recovery code: <?php echo $user['ga'] ?></p>
	...
	<p><input type="submit" name="deactivate" id="deactivate" value="Disable 2FA" onclick="return confirm('Are you sure you want to do that?');"></p>
	...
	$url =  sprintf("otpauth://totp/%s?secret=%s&issuer=YOUR_FORUM", $user['username'], $user['ga']); //change 2fa name YOUR_FORUM
...
<p>Refresh this page if code not valid.</p>
	...
	<p><input type="submit" name="activate" id="activate" value="Enable 2FA" onclick="return confirm('Are you sure you want to do that?');"></p>
...
<?php
	...
	else if ($section == 'premium')
	{
		...
		if($user['group_id'] == 4 && $pun_user['g_id'] < 2){
			message("User not premium.");
			
		}
				$page_title = array(pun_htmlspecialchars($pun_config['o_board_title']), $lang_common['Profile'], "Premium user options ");
		define('PUN_ACTIVE_PAGE', 'profile');
		require PUN_ROOT.'header.php';

		generate_profile_menu('premium');
...
?>
	<div class="blockform">
		...
		if($user['group_id'] == 4 && $pun_user['g_id'] <= 2){
					...
					echo pun_htmlspecialchars($user['username']).' - Premium user options (Warning: user not premium)' ;
		} else {
			
						echo pun_htmlspecialchars($user['username']).' - Premium user options' ;
		}
...
<p>Subscription for Counter-Strike: Global Offensive expires on <?php echo $resultdate; ?>.</p>						</div>
					</fieldset>
				</div>
				<div class="inform">
					<fieldset>
						<legend>Request a hardware ID reset</legend>
						<div class="infldset">
							<p>Enter the specific component(s) that changed.</p><input id="hwid_reason" type="text" name="form[hwid_reason]" value="" size="40" maxlength="40">
							<p><input type="submit" name="reset_hwid" value="Reset HWID"></p>
						</div>
					</fieldset>
				</div>
				<div class="inform">
					<fieldset>
						<legend>Request a discord ID reset</legend>
						<div class="infldset">
							...
							<p>Enter the reason for this request.</p><input id="discord_reason" type="text" name="form[discord_reason]" value="" size="40" maxlength="40">
							<p><input type="submit" name="update" value="Reset discord ID"></p>
						</div>
					</fieldset>
				</div>
					...
					<fieldset>
						...
						<?php
						$mytopid = $user['id'];
	$result4411 = $db->query("SELECT `token` FROM `users` WHERE `id`='$mytopid'") or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());

					$invbylol = $db->fetch_assoc($result4411);
			
$invbylol = $invbylol['token'];
						
				if($invbylol == "" || $invbylol == null){		
						?>
						<p>You do not have token key for api</p>
				<?php }else {
					...
					echo "<p>$invbylol</p>";
						...
						<?php if($invbylol == "" || $invbylol == null){		
						

						?>
						 <p><input type="submit" class="post" name="token" value="Create"></p>
 ...
 <script type="text/javascript">
	...
	<?php }else {
				?>	
				 <p><input type="submit" class="post" name="token" value="Update"></p>
	...
	<?php }
				...
				<div class="inform">
					<fieldset>
						<legend>Download sandboxie fix</legend>
						<div class="infldset">
						<p>Use your forum password for unpacking.</p>
							<p><input type="submit" class="post" name="fix" value="Download"></p>
							 <script type="text/javascript">
            jQuery(document).ready(function($){
                $(".post").on("click",function(){
                    $.ajax({
				...
				<div class="inform">
					<fieldset>
						<legend>Download latest client</legend>
						<div class="infldset">
						<p>Use your forum password for unpacking.</p>
							<p><input type="submit" class="post" name="client" value="Download"></p>
						 <script type="text/javascript">
            jQuery(document).ready(function($){
                $(".post").on("click",function(){
                    $.ajax({
				...
				<div class="inform">
					<fieldset>
						...
						<?php 
	$mytopid = $user['id'];
	$result4478 = $db->query("SELECT * FROM `codes` WHERE `by`='$mytopid' AND `used`='0'") or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
		...
		<form id="profile7" method="post" action="profile.php?section=admin&amp;id=<?php echo $id ?>">
					...
					<table><thead>
						<tr>
							...
							<th class="tc3" scope="col">Generated by</th>
							<th class="tc3" scope="col">Generated at</th>
							<th class="tc3" scope="col">Used at</th>
					
						</tr>
						<tbody>
						<?php 
						
						$result44 = $db->query("SELECT * FROM `codes` WHERE `by`='$id'") or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
						...
						$itemka = $item['code'];
						$result447 = $db->query("SELECT * FROM `codes` WHERE `code`='$itemka'") or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
                        $invby = $db->fetch_assoc($result447);
						
						$usercode = $invby['user'];
						$userused = $invby['used'];
						$useradmin = $invby['admin'];
...
$usercodel = $db->query("SELECT `username`, `group_id` FROM `users` WHERE `id`='$usercode'") or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
 $usercodel = $db->fetch_assoc($usercodel);    
 $usercode11 = $usercodel['username']; 
$usercode12 = $usercodel['group_id'];  
	echo "<a href='profile.php?id=$usercode'>".colorize_group($usercode11,$usercode12)."</a>";
...
echo "-";
								...
								$useradminl = $db->query("SELECT `username`, `group_id` FROM `users` WHERE `id`='$useradmin'") or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
 $useradminl = $db->fetch_assoc($useradminl);    
  $usercadmin11 = $useradminl['username']; 
$useradmin12 = $useradminl['group_id'];   
								echo "<a href='profile.php?id=$useradmin'>".colorize_group($usercadmin11,$useradmin12)."</a>";


								?></td>
									<td class="tc3"><?php 
if($item['date'] != null)	{							
			$codedate = new DateTime($item['date']);				
$codedate = format_time($codedate->getTimestamp(), false, null, null, false, true);
...
<?php endforeach; ?> 
						...
						<tr>
							<td colspan="5" style="text-align: center;">User don't have any codes.</td>
						<tr>
						<?php endif; ?> 
					...
					<br>
						<p><input name="beta" value="pass" hidden /></p>
						
							<p><input type="submit" name="generate" value="Generate new code" onclick="return confirm('Are you sure you want to do that?');" /></p>
						</div>
					</fieldset>
				</div>
...
$todayrn = $datenow->format('Y-m-d H:i:s');
...
if($user['csgo'] != null){
							...
							?>
							<label>Expire time<br /><input id="sub" type="text" name="form[sub]" value="<?php echo pun_htmlspecialchars($user['csgo']) ?>" size="40" maxlength="75" /><br /></label>
<p>Format: Y-m-d H:i:s</p>
<p>Time zone: (UTC-05:00) Eastern</p>

							<?php } else { ?>


							<label>Expire time<br /><input id="sub" type="text" name="form[sub]" value="<?php echo pun_htmlspecialchars($todayrn) ?>" size="40" maxlength="75" /><br /></label>
<p>Format: Y-m-d H:i:s</p>
<p>Time zone: (UTC-05:00) Eastern</p>

<?php } ?>
								<p><input type="submit" name="update" value="<?php echo $lang_common['Submit'] ?>" /> <a href="profile.php?section=admin&id=<?php echo $user['id'] ?>&email=1">Send email</a></p>
