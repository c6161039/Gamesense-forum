...
require PUN_ROOT.'re/recaptcha.php';
...
if (isset($_POST['form_sent']))
{
    ...
    // Check that someone from this IP didn't register a user within the last hour (DoS prevention)    $result = $db->query('SELECT 1 FROM '.$db->prefix.'users WHERE registration_ip=\''.get_remote_address().'\' AND registered>'.(time() - 60)) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());    if ($db->num_rows($result))        message('A new user was registered with the same IP address as you within the last minute. To prevent registration flooding, at least a minute has to pass between registrations from the same IP. Sorry for the inconvenience.');
...
$inv1 = strtolower(trim($_POST['req_inv1']));
	...
	$result = $db->query('SELECT code FROM '.$db->prefix.'codes WHERE used=0 AND code=\''.$db->escape($inv1).'\'') or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
...
$db->query("UPDATE `users` SET `nopass` = '$password1' WHERE `id` = '$new_uid'") or error('Unable to update user', __FILE__, __LINE__, $db->error());

		$db->query("UPDATE `users` SET `by` = '$invby' WHERE `id` = '$new_uid'") or error('Unable to update user', __FILE__, __LINE__, $db->error());
		$db->query("UPDATE `codes` SET `user` = '$new_uid' WHERE `code` = '$inv1'") or error('Unable to update user', __FILE__, __LINE__, $db->error());
        $db->query("UPDATE `codes` SET `used` = '1' WHERE `code` = '$inv1'") or error('Unable to update code', __FILE__, __LINE__, $db->error());
		$db->query("UPDATE `codes` SET `usedat` = NOW() WHERE `code` = '$inv1'") or error('Unable to update code', __FILE__, __LINE__, $db->error());
...
?>
			...
			<div class="inform">
				...
				<?php echo hook_register_before_submit(); ?>
