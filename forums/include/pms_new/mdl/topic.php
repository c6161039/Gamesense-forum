...
{
?>
			<li><span>»&#160;</span><a href="pmsnew.php?mdl=<?php echo $mmodul.'&amp;sid='.$to_user['id'] ?>"><?php echo colorize_group($to_user['username'],$to_user['group_id']) ?></a></li>
<?php
}
?>
...
while ($cur_post = $db->fetch_assoc($result))
{
	...
	if ($pun_user['g_view_users'] == '1' && $cur_post['g_id'] != PUN_GUEST)
		$username = '<a href="profile.php?id='.$cur_post['poster_id'].'">'.colorize_group($cur_post['username'], $cur_post['group_id']).'</a>';
	else
		$username = colorize_group($cur_post['username'], $cur_post['group_id']);
			...
			<div class="box">
				<div class="inbox">
					<div class="postbody">
						<div class="postleft">
							<dl>
								<dt><strong><?php echo colorize_group($cur_post['username'], $cur_post['group_id']) ?></strong></dt>
