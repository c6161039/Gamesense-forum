...
if (!$pun_user['is_guest'])
	$result = $db->query('SELECT t.subject, t.closed, t.poll_type, t.poll_time, t.poll_term, t.poll_kol, t.num_replies, t.sticky, t.first_post_id, f.id AS forum_id, f.forum_name, f.moderators, fp.post_replies, s.user_id AS is_subscribed FROM '.$db->prefix.'topics AS t INNER JOIN '.$db->prefix.'forums AS f ON f.id=t.forum_id LEFT JOIN '.$db->prefix.'topic_subscriptions AS s ON (t.id=s.topic_id AND s.user_id='.$pun_user['id'].') LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND t.id='.$id.' AND t.moved_to IS NULL') or error('Unable to fetch topic info', __FILE__, __LINE__, $db->error());
else
...
$result = $db->query('SELECT u.email, u.title, u.url, u.location, u.signature, u.email_setting, u.num_posts, u.registered, u.admin_note, p.id, p.poster AS username, p.poster_id, p.poster_ip, p.poster_email, p.message, p.hide_smilies, p.posted, p.edited, p.edited_by, p.likes, g.g_id, g.g_user_title, g.g_promote_next_group, g.g_pm, u.messages_enable, o.user_id AS is_online FROM '.$db->prefix.'posts AS p INNER JOIN '.$db->prefix.'users AS u ON u.id=p.poster_id INNER JOIN '.$db->prefix.'groups AS g ON g.g_id=u.group_id LEFT JOIN '.$db->prefix.'online AS o ON (o.user_id=u.id AND o.user_id!=1 AND o.idle=0) WHERE p.id IN ('.implode(',', $post_ids).') ORDER BY p.id', true) or error('Unable to fetch post info', __FILE__, __LINE__, $db->error());
					...
					<div class="postmsg">
						...
						foreach($likes as $ids => $like_username)
						{
							$like_username = pun_htmlspecialchars($like_username);
								$ids2 = $db->query('SELECT group_id FROM '.$db->prefix.'users WHERE id='.$ids) or error('Unable to fetch user data', __FILE__, __LINE__, $db->error());
	if ($db->num_rows($ids2))
	{
		$ids2 = $db->fetch_assoc($ids2);
	}	
							if ($pun_user['g_view_users'] == '1')
								$like_info = '<a href="profile.php?id='.$ids.'">'.colorize_group($like_username,$ids2['group_id']).'</a>';
							else
								$like_info = colorize_group($like_username,$ids2['group_id']);
