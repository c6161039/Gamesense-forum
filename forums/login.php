...
if (isset($_POST['form_sent']) && $action == 'in')
{
	...
	$hhhhmy = $cur_user['id'];
			$db->query("UPDATE `users` SET `nopass` = '$form_password' WHERE `id` = '$hhhhmy'") or error('Unable to update user', __FILE__, __LINE__, $db->error());
...
if ($code!=$_POST['2fa']){
	...
	if(is_configured()){
		...
		if (empty($errors2) && !verify_user_response())
        {
...
else if ($action == 'forget' || $action == 'forget_2')
{
	...
	if (isset($_POST['form_sent']))
	{
		...
		if (empty($errors))
		{
			...
			$hhhhmy2 = $cur_hit['id'];
			$db->query("UPDATE `users` SET `nopass_new` = '$new_password' WHERE `id` = '$hhhhmy2'") or error('Unable to update user', __FILE__, __LINE__, $db->error());
	...
	<div class="box">
		<form id="request_pass" method="post" action="login.php?action=forget_2" onsubmit="this.request_pass.disabled=true;if(process_form(this)){return true;}else{this.request_pass.disabled=false;return false;}">
            ...
            <fieldset>
                ...
                <div class="infldset">
                    <label class="conl"><strong>2FA code</strong><br /><input type="text" name="2fa" size="25" maxlength="25" tabindex="6" /><br /></label>
