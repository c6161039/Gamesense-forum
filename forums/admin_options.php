...
$csgoupdates = pun_trim($_POST['form']['csgo']);
	if($csgoupdates != null)
	$db->query("UPDATE `updates` SET `updated` = '$csgoupdates' WHERE `id` = '1'") or error('Unable to update', __FILE__, __LINE__, $db->error());
	
	$csgobetaupdates = pun_trim($_POST['form']['csgo_beta']);
	if($csgobetaupdates != null)
	$db->query("UPDATE `updates` SET `updated` = '$csgobetaupdates' WHERE `id` = '2'") or error('Unable to update', __FILE__, __LINE__, $db->error());


	$csgoversion = pun_trim($_POST['form']['csgo_version']);
	if($csgoversion != null)
	$db->query("UPDATE `versions` SET `version` = '$csgoversion' WHERE `id` = '2'") or error('Unable to update', __FILE__, __LINE__, $db->error());
	
	
	$loaderversion = pun_trim($_POST['form']['loader_version']);
	if($loaderversion != null)
	$db->query("UPDATE `versions` SET `version` = '$loaderversion' WHERE `id` = '1'") or error('Unable to update', __FILE__, __LINE__, $db->error());
...
?>
 ...
 $updates_csgo = $db->query("SELECT * FROM `updates` WHERE `id`='1'") or error('Unable to fetch info', __FILE__, __LINE__, $db->error());
 $updates_csgo = $db->fetch_assoc($updates_csgo);   
	 $updates_csgo_beta = $db->query("SELECT * FROM `updates` WHERE `id`='2'") or error('Unable to fetch info', __FILE__, __LINE__, $db->error());
 $updates_csgo_beta = $db->fetch_assoc($updates_csgo_beta);  	
 ...
 $version_csgo = $db->query("SELECT * FROM `versions` WHERE `id`='2'") or error('Unable to fetch info', __FILE__, __LINE__, $db->error());
 $version_csgo = $db->fetch_assoc($version_csgo);   
						...
						?>
							<table class="aligntop" cellspacing="0">
								<tr>
									...
									<td>
										<input type="text" name="form[csgo]" size="50" maxlength="255" value="<?php echo $updates_csgo['updated'] ?>" />
										<span>Last update date.</span>
									</td>
								</tr>
								<tr>
									<th scope="row">CS:GO Beta</th>
									<td>
										<input type="text" name="form[csgo_beta]" size="50" maxlength="255" value="<?php echo $updates_csgo_beta['updated'] ?>" />
										<span>Last update date.</span>
									</td>
								</tr>
								<tr>
									<th scope="row">CS:GO Version</th>
									<td>
										<input type="text" name="form[csgo_version]" size="50" maxlength="255" value="<?php echo $version_csgo['version'] ?>" />
										<span>Current version (0.0 - disable game).</span>
									</td>
								</tr>
								<tr>
									<th scope="row">Loader Version</th>
									<td>
										<input type="text" name="form[loader_version]" size="50" maxlength="255" value="<?php echo $version_loader['version'] ?>" />
										<span>Current version (0.0 - disable loader).</span>
									</td>
								</tr>
